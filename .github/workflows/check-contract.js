...
async function main() {
  try {
    ...
    const verificationFilter = contract.filters.TwitterVerificationResult();
    const allVerificationEvents = await contract.queryFilter(verificationFilter, dayAgo, now);
    // Filter successful verifications in memory
    const verificationEvents = allVerificationEvents.filter(event => event.args.isSuccess);
    ...
    console.log('Processing minting transfers...');
    ...
    console.log('Getting Twitter user IDs...');
    ...
    console.log('Getting Twitter usernames...');
